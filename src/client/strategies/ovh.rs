...
impl OVHStrategy {
    pub fn new(config: Option<OVHConfig>) -> Self {
        ...
        if config.is_none() { panic!("ovh config cannot be None!") }
        OVHStrategy { config: config.unwrap() }

    }
}

impl Strategy for OVHStrategy {
    fn query(&self, client: &reqwest::blocking::Client, address: &str) -> Result<(), StrategyError> {
